...
def determine_if_tree_is_visible(
    a_tree_map: dict, coordinates: tuple, map_height: int, map_width: int
) -> bool:
    ...
    if (
        coordinates[0] == 0
        or coordinates[0] == (map_width - 1)
        or coordinates[1] == 0
        or coordinates[1] == (map_height - 1)
    ):
        return True
    ...
    return (
        all(test_values_north)
        or all(test_values_south)
        or all(test_values_east)
        or all(test_values_west)
    )
...
def compute_scenic_score(
    ...
    for y in range(coordinates[1] + 1, map_height):
        ...
        if a_tree_map[(coordinates[0], y)] >= this_tree_height:
            break
    return (
        scenic_value_north * scenic_value_south * scenic_value_east * scenic_value_west
    )


if __name__ == "__main__":
    # Assume positive y is downwards and positive x is to the right.
    tree_map_text = input_per_line("08_input_test.txt")
    ...
    for this_x in range(maximum_width):
        for this_y in range(maximum_height):
            visible_trees.append(
                determine_if_tree_is_visible(
                    tree_map, (this_x, this_y), maximum_height, maximum_width
                )
            )
            scenic_values.append(
